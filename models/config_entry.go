...
type ConfigEntry struct {
	...
	// The value type.
	// Enum: [STRING DEVICE PERMISSION MODE SCENE MESSAGE]
...
func (m *ConfigEntry) Validate(formats strfmt.Registry) error {
	...
	if err := m.validateMessageConfig(formats); err != nil {
		res = append(res, err)
	}
	...
	if err := m.validateSceneConfig(formats); err != nil {
		res = append(res, err)
	}
...
func (m *ConfigEntry) validateMessageConfig(formats strfmt.Registry) error {

	if swag.IsZero(m.MessageConfig) { // not required
		return nil
	}

	if m.MessageConfig != nil {
		if err := m.MessageConfig.Validate(formats); err != nil {
			if ve, ok := err.(*errors.Validation); ok {
				return ve.ValidateName("messageConfig")
			}
			return err
		}
	}
...
func (m *ConfigEntry) validateSceneConfig(formats strfmt.Registry) error {

	if swag.IsZero(m.SceneConfig) { // not required
		return nil
	}

	if m.SceneConfig != nil {
		if err := m.SceneConfig.Validate(formats); err != nil {
			if ve, ok := err.(*errors.Validation); ok {
				return ve.ValidateName("sceneConfig")
			}
			return err
		}
	}
...
func init() {
	var res []string
	if err := json.Unmarshal([]byte(`["STRING","DEVICE","PERMISSION","MODE","SCENE","MESSAGE"]`), &res); err != nil {
		panic(err)
	}
	for _, v := range res {
...
const (
	...
	ConfigEntryValueTypeSCENE string = "SCENE"

	// ConfigEntryValueTypeMESSAGE captures enum value "MESSAGE"
	ConfigEntryValueTypeMESSAGE string = "MESSAGE"
)

// prop value enum
func (m *ConfigEntry) validateValueTypeEnum(path, location string, value string) error {
	if err := validate.EnumCase(path, location, value, configEntryTypeValueTypePropEnum, true); err != nil {
		return err
	}
	return nil
}
