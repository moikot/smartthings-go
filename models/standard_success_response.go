// Code generated by go-swagger; DO NOT EDIT.

package models

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"encoding/json"

	"github.com/go-openapi/errors"
	"github.com/go-openapi/strfmt"
	"github.com/go-openapi/swag"
	"github.com/go-openapi/validate"
)

// StandardSuccessResponse {"status": "success"}
//
// swagger:model StandardSuccessResponse
type StandardSuccessResponse struct {

	// status
	// Enum: [success]
	Status *string `json:"status,omitempty"`
}

// Validate validates this standard success response
func (m *StandardSuccessResponse) Validate(formats strfmt.Registry) error {
	var res []error

	if err := m.validateStatus(formats); err != nil {
		res = append(res, err)
	}

	if len(res) > 0 {
		return errors.CompositeValidationError(res...)
	}
	return nil
}

var standardSuccessResponseTypeStatusPropEnum []interface{}

func init() {
	var res []string
	if err := json.Unmarshal([]byte(`["success"]`), &res); err != nil {
		panic(err)
	}
	for _, v := range res {
		standardSuccessResponseTypeStatusPropEnum = append(standardSuccessResponseTypeStatusPropEnum, v)
	}
}

const (

	// StandardSuccessResponseStatusSuccess captures enum value "success"
	StandardSuccessResponseStatusSuccess string = "success"
)

// prop value enum
func (m *StandardSuccessResponse) validateStatusEnum(path, location string, value string) error {
	if err := validate.EnumCase(path, location, value, standardSuccessResponseTypeStatusPropEnum, true); err != nil {
		return err
	}
	return nil
}

func (m *StandardSuccessResponse) validateStatus(formats strfmt.Registry) error {

	if swag.IsZero(m.Status) { // not required
		return nil
	}

	// value enum
	if err := m.validateStatusEnum("status", "body", *m.Status); err != nil {
		return err
	}

	return nil
}

// MarshalBinary interface implementation
func (m *StandardSuccessResponse) MarshalBinary() ([]byte, error) {
	if m == nil {
		return nil, nil
	}
	return swag.WriteJSON(m)
}

// UnmarshalBinary interface implementation
func (m *StandardSuccessResponse) UnmarshalBinary(b []byte) error {
	var res StandardSuccessResponse
	if err := swag.ReadJSON(b, &res); err != nil {
		return err
	}
	*m = res
	return nil
}
