// Code generated by go-swagger; DO NOT EDIT.

package models

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"github.com/go-openapi/errors"
	"github.com/go-openapi/strfmt"
	"github.com/go-openapi/swag"
)

// UpdateSignatureTypeRequest A request object used to switch the signing mechanism for authorizing SmartApp callbacks.
//
// swagger:model UpdateSignatureTypeRequest
type UpdateSignatureTypeRequest struct {

	// signature type
	SignatureType SignatureType `json:"signatureType,omitempty"`
}

// Validate validates this update signature type request
func (m *UpdateSignatureTypeRequest) Validate(formats strfmt.Registry) error {
	var res []error

	if err := m.validateSignatureType(formats); err != nil {
		res = append(res, err)
	}

	if len(res) > 0 {
		return errors.CompositeValidationError(res...)
	}
	return nil
}

func (m *UpdateSignatureTypeRequest) validateSignatureType(formats strfmt.Registry) error {

	if swag.IsZero(m.SignatureType) { // not required
		return nil
	}

	if err := m.SignatureType.Validate(formats); err != nil {
		if ve, ok := err.(*errors.Validation); ok {
			return ve.ValidateName("signatureType")
		}
		return err
	}

	return nil
}

// MarshalBinary interface implementation
func (m *UpdateSignatureTypeRequest) MarshalBinary() ([]byte, error) {
	if m == nil {
		return nil, nil
	}
	return swag.WriteJSON(m)
}

// UnmarshalBinary interface implementation
func (m *UpdateSignatureTypeRequest) UnmarshalBinary(b []byte) error {
	var res UpdateSignatureTypeRequest
	if err := swag.ReadJSON(b, &res); err != nil {
		return err
	}
	*m = res
	return nil
}
