...
func init() {
	var res []string
	if err := json.Unmarshal([]byte(`["USER","SYSTEM","IMPLICIT"]`), &res); err != nil {
		panic(err)
	}
	for _, v := range res {
...
const (
	...
	OwnerOwnerTypeSYSTEM string = "SYSTEM"

	// OwnerOwnerTypeIMPLICIT captures enum value "IMPLICIT"
	OwnerOwnerTypeIMPLICIT string = "IMPLICIT"
)

// prop value enum
func (m *Owner) validateOwnerTypeEnum(path, location string, value string) error {
	if err := validate.EnumCase(path, location, value, ownerTypeOwnerTypePropEnum, true); err != nil {
		return err
	}
	return nil
}
